...
int getBit(char * data, int n)
{
	int charNumber = n / 8;
	int rNumber = n % 8;
	
	int mask = 1 << (7 - rNumber);
	if(array[charNumber] & mask)
		return 1;
	else
		return 0;
}

void setBit(char * data, int n , int save)
{
	int charNumber = n / 8;
	int rNumber = n % 8;
	int mask = 1 << (7 - rNumber);
...
struct inode{
	...
	int sin;
	int dou;
	struct d_block *directBlock;
	struct d_block *SindirectBlock;
};
void makeSingleIndirect(Inode * inode)
{
	inode->SindirectBlock = getEmptyDataBlock();

};
void makeDoubleIndirect(Inode * inode)
{
	inode -> DindirectBlock = getEmptyDataBlock();
	for(int i = 0 ; i < 102; i++)
	{
		int num = getEmptyDataBlock();
		for(int j = 0 ; j < 10 ;j++)
		{
			setBit(inode->DindirectBlock -> data, i*10+j, getBit(num,j+22));
		}
	}
}
void addDataBlock(Inode *inode , DataBlock * dataBlock)
{
	int dataBlockNumber = inode -> (file_size+128) / 128;
	if(dataBlockNumber == 0)
		inode -> directBlock = dataBlock;
	dataBlockNumber -= 1;
	makeSingleIndirect(Inode *inode);
	if(dataBlockNumber <= 102)
	{
		int startBit = dataBlockNumber * 10;
		int num = dataBlock -> number;
		for(int i = 0; i < 10; i++)
		{
			setBit(inode -> SindirectBlock -> data /*char [128]*/, startBit + i ,getBit(num,i + 22));
		}
		return ;
	}
	makeDoubleIndirect(Inode * inode);
	else if(dataBlockNumber > 102)
	{
		int num2 = dataBlock -> number;
		int a=dataBlockNumber/102;
		int b=dataBlockNumber % 102;
		for(int i = 0; i<10; i++)
		{
			setBit(num2, i +22,getBit(inode->DindirectBlock->data, a*10+i));

		}
		DataBlock * get = fileSystem->datablock[a];
		for(int i= 0; i<10; i++)
		{
			setBitget, b*10+i,getBit(dataBlock->number, 22+i);
		
		}
		return ;

	}
//이중 데이터블록에서 간접데이터블록번호 알아오고 그번호로 데이터블럭 얻어온 다음에 그데이터블럭에다가 숫자를 써야하
